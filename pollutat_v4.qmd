...
p1 <- ggplot(records_by_year, aes(x = year, y = n)) +
  ...
  labs(
    title = "Number of Records by Year (2000–2023)",
    x = "Year", y = "Record Count"
  ) +
  common_theme

p2 <- plot_usmap(data = records_by_state, values = "n", color = "white") +
  scale_fill_continuous(
    low = "#D4E6F1", high = "#1B4F72",
    name = "Record Count", label = scales::comma
  ) +
  labs(
...
p3 <- records_by_state %>%
  arrange(desc(n)) %>%
  slice_head(n = 15) %>%               # 等价 head(15)，更明确一点
  ggplot(aes(x = reorder(state, n), y = n / 1000)) +
  geom_col(fill = "#1B4F72") +
  coord_flip(clip = "off") +           # 关键：防止右边被裁掉
  scale_y_continuous(
  ...
  labs(
    ...
    x = NULL,
    y = "Record Count (thousands)"
  ) +
  common_theme +
  theme(
    axis.text.y  = element_text(size = 8),
    plot.margin  = margin(t = 5, r = 20, b = 5, l = 5)  # 右边多留一点空
  )

library(patchwork)

layout <- (p1) /
  ((p2 | p3) + plot_layout(widths = c(1, 1.5))) +
...
Based on the regional comparison of the past decade (2010–2023), we observe clear geographic patterns in U.S. air pollution:
...
Overall, the Northeast & North Central are the most polluted regions across multiple pollutants, while the West and South tend to be relatively cleaner in most pollutants except ozone.
