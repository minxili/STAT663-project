...
n_states <- df_clean %>% distinct(state) %>% nrow()
year_min <- min(df_clean$year, na.rm = TRUE)
year_max <- max(df_clean$year, na.rm = TRUE)
...
#--------------------------
# 5. STL decomposition (O3/NO2/SO2/CO)
#--------------------------
...
make_animated_map <- function(poll) {
  ...
  plot_ly(
    ...
    text = ~paste0(
      ...
      "Year: ", year, "<br>",
      toupper(poll), ": ",
      ifelse(is.na(value), "No data", round(value, 3))
    ),
    hoverinfo   = "text",
    colorscale  = "Reds",
    showscale   = TRUE,
...
#==========================
ui <- navbarPage(
  title = "U.S. Air Quality Dashboard (2000–2022)",
  ...
  tabPanel(
    ...
    fluidPage(
      br(),
      h3("Overview of U.S. Air Quality (2000–2022)"),
      p("This dashboard summarizes long-term trends, spatial patterns, and seasonal behaviors of four key pollutants: O\u2083, NO\u2082, SO\u2082, and CO."),
      br(),
      fluidRow(
        column(
          4,
      ...
      h4("How to navigate"),
      tags$ul(
        tags$li(strong("Time Trends:"), " Annual and seasonal patterns of average AQI."),
        tags$li(strong("Spatial Patterns:"), " State-level maps and regional boxplots."),
        tags$li(strong("Animated Map:"), " Year-by-year animated choropleth for each pollutant."),
        tags$li(strong("Correlation:"), " Spearman correlation heatmap among pollutants."),
        tags$li(strong("PCA Overview:"), " PCA-based summary of overall pollution burden and composition."),
        tags$li(strong("STL Decomposition:"), " Seasonal-trend decomposition for daily AQI.")
      )
    )
  ),
  tabPanel(
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        checkboxGroupInput(
          "trend_pollutants",
          "Pollutants to display (line plots):",
          choices  = c("O3", "NO2", "SO2", "CO"),
          selected = c("O3", "NO2", "SO2", "CO"),
          inline   = TRUE
        )
      ),
      mainPanel(
        plotOutput("trendPlot",   height = "320px"),
        br(),
        plotOutput("heatmapPlot", height = "360px")
      )
    )
  ),
  ...
  tabPanel(
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        helpText("STL decomposition for daily mean AQI (Trend + Seasonality + Remainder).")
      ),
      mainPanel(
  ...
  tabPanel(
    ...
    fluidPage(
      ...
      p("This dashboard presents a comprehensive analysis of U.S. air pollution from 2000 to 2022, focusing on ozone (O₃), nitrogen dioxide (NO₂), sulfur dioxide (SO₂), and carbon monoxide (CO). Three major analytical themes — temporal trends, spatial variability, and multivariate structure — reveal several consistent insights across pollutants."),
      
      h4("1. Temporal Trends"),
      tags$ul(
        tags$li("Ozone (O₃) shows strong, regular seasonality with sharp summer peaks each year."),
        tags$li("NO₂, SO₂, and CO exhibit clear long-term declines, reflecting successful regulatory and environmental policy impacts."),
        tags$li("While combustion-related pollutants decreased steadily, ozone levels remained relatively stable over the past decade.")
      ),
      
      h4("2. Spatial Patterns"),
      tags$ul(
        tags$li("Western and southwestern states tend to experience higher O₃ levels compared to the eastern U.S."),
        tags$li("NO₂, SO₂, and CO concentrations are generally higher in more urbanized and industrial regions, especially along the East Coast and Midwest."),
        tags$li("Regional boxplots highlight distinct distributions of pollutant levels across the Northeast, South, Midwest, and West.")
      ),
      
      h4("3. Multivariate Structure"),
      tags$ul(
        tags$li("Spearman correlations reveal moderate positive associations among combustion-related pollutants (NO₂, SO₂, CO)."),
        tags$li("PCA shows that PC1 represents overall pollution burden, separating cleaner states from more polluted states."),
        tags$li("PC2 differentiates ozone-dominated regions (high O₃, low combustion pollutants) from combustion-dominated regions (high NO₂/SO₂/CO).")
      ),
      
      h4("4. Time Series Behavior (STL)"),
      tags$ul(
        tags$li("STL decomposition confirms strong seasonal cycles for O₃, while the trend component fluctuates mildly across years."),
        tags$li("Noise components (remainders) remain relatively small, suggesting stable long-term pollutant dynamics.")
      ),
      
      h4("Overall Conclusion"),
      p("Across all methods, the evidence indicates significant improvements in air quality for NO₂, SO₂, and CO since 2000, while ozone remains the most persistent and seasonally variable pollutant. Spatial differences highlight ongoing regional disparities in pollutant sources and atmospheric behavior. These patterns collectively illustrate both progress in air quality management and remaining challenges, particularly regarding ozone formation and regional transport.")
    )
  )
)
...
server <- function(input, output, session) {
  ...
  output$trendPlot <- renderPlot({
    ...
    plot_data <- trend_all %>%
      filter(pollutant %in% input$trend_pollutants)
  ...
  output$animatedMap <- renderPlotly({
    ...
    poll <- tolower(input$ani_pollutant)
